...
namespace Wby.Demo.Api.ApiManager
{
    public class UserManager : IUserManager
    {
        ...
        public async Task<ApiResponse> GetAll(QueryParameters param)
        {
            try
            {
                var models = await work.GetRepository<User>().GetPagedListAsync(predicate: x =>
        ...
        public async Task<ApiResponse> Login(LoginDto param)
        {
            try
            {
                var model = await work.GetRepository<User>().GetFirstOrDefaultAsync(predicate: x => x.Account == param.Account
                   && x.Password == param.PassWord);
                if (model != null)
                {
                    //获取所属权限
                    var context = work.GetDbContext<WbyContext>();
                    if (model.FlagAdmin == 1)
                    {
                        ...
                        return new ApiResponse(200, new { User = model, Menus = data.ToList() });
                    }
                    else
                    {
                        var data = from a in context.GroupFuncs
                                   ...
                                   select new { d.MenuName, d.MenuCaption, d.MenuNameSpace, a.Auth };
        ...
        public async Task<ApiResponse> Save(UserDto param)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (dbUser == null)
                        return new ApiResponse(201, "该用户已不存在！");
