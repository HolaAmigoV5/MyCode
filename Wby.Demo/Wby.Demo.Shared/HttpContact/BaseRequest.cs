        ...
        public string GetPropertiesObject()
        {
            ...
            if (propertyArray != null && propertyArray.Length > 0)
            {
                foreach (PropertyInfo property in propertyArray)
                {
                    ...
                    if (pvalue != null && pvalue.GetType().Namespace == "Wby.Demo.Shared.Common.Query")
                    {
                        ...
                        (pvalue.GetType().Namespace.Contains("Wby.Demo.Shared.Dto") ||
                        pvalue.GetType().Namespace.Contains("Wby.Demo.Shared.HttpContact")))
                    {
                        //当属性为对象得情况下, 进行序列化
