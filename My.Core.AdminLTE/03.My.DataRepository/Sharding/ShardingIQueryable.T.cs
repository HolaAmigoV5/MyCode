...
{
    /// <summary>
    /// 描述：IShardingQueryable接口的实现
    /// 作者：wby 2019/10/9 10:35:28
    /// </summary>
    public class ShardingQueryable<T> : IShardingQueryable<T> where T : class, new()
    {
        ...
        private List<dynamic> GetStaticData(Func<IQueryable, dynamic> access, IQueryable<T> newSource = null)
        {
        ...
        {
            return GetStaticData(x => My.Util.Extention.DynamicSum(x, selector)).Sum(x => (decimal?)x);
        }

        private dynamic DynamicAverage(dynamic selector)
        {
            var list = GetStaticData(x => new KeyValuePair<int, dynamic>(x.Count(), My.Util.Extention.DynamicSum(x, selector)))
                .Select(x => (KeyValuePair<int, dynamic>)x).ToList();
        ...
        public List<T> ToList()
        {
            ...
            tables.GroupBy(x => x.conString).Select(x => x.Key).ForEach(x => lockMap.Add(x, new object()));
            tables.ForEach(aTable =>
            {
                tasks.Add(Task.Run(() =>
                {
                    ...
                    void Run()
                    {
                        list = newQ.CastToList<object>().Select(x => x.ChangeType<T>()).ToList();
                    }
                }));
            });
        ...
        public bool Any(Expression<Func<T, bool>> predicate)
        {
            ...
            return GetStaticData(x => x.Any(), newSource).Any(x => x == true);
        }
        ...
        public int count()
        {
            return GetStaticData(x => x.Count()).Sum(x => (int)x);
        }
        ...
        {
            return GetStaticData(x => x.Max(selector)).Max(x => (TResult)x);
        }

        public TResult Min<TResult>(Expression<Func<T, TResult>> selector)
        {
            return GetStaticData(x => x.Min(selector)).Min(x => (TResult)x);
        }
