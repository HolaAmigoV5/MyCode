...
{
    /// <summary>
    /// 描述：ShardingRepository
    /// 作者：wby 2019/9/30 16:07:54
    /// </summary>
    public class ShardingRepository : IShardingRepository
    {
        ...
        private List<(object targetObj, IRepository targetDb)> GetMapConfigs<T>(List<T> entities)
        {
            ...
            {
                (string tableName, string conString, DatabaseType dbType) = ShardingConfig.Instance.GetWriteTables(typeof(T).Name, aEntity);
        ...
        private void WriteTable<T>(List<T> entities, Action<object, IRepository> accessData)
        {
            ...
            void Run()
            {
                ...
                {
                    tasks.Add(Task.Run(() => { accessData(aConfig.targetObj, aConfig.targetDb); }));
                });
        ...
        {
            var allTables = ShardingConfig.Instance.GetAllWriteTables(typeof(T).Name);
            var allDbs = allTables.Select(x => new
            {
        ...
        /// </summary>
        /// <typeparam name="T">实体泛型</typeparam>
        /// <param name="whereExpre">实体对象集合</param>
        /// <param name="set">属性</param>
