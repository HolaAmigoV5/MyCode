...
namespace My.Util
{
    /// <summary>
    /// 描述：父子数据相互映射类
    /// 注：父子数据都必须唯一,即所有父键唯一,所有子键唯一,
    /// 唯一的子键能确定对应的父键,唯一的父键能确定其拥有的子键集合
    /// 作者：wby 2019/10/22 13:29:44
    /// </summary>
    public class ParentChildrenMapping
    {
        #region 私有成员
        private ConcurrentDictionary<string, string> _cToP { get; set; } = new ConcurrentDictionary<string, string>();
        private ConcurrentDictionary<string, ConcurrentBag<string>> _pToC { get; set; } =
        ...
        public void AddChild(string parentId, string childId)
        {
            if (string.IsNullOrEmpty(parentId))
                throw new ArgumentNullException("父键不能为Null或空");
            if (string.IsNullOrEmpty(childId))
                throw new ArgumentNullException("子键不能为Null或空");
        ...
        {
            if (string.IsNullOrEmpty(childId))
                throw new ArgumentException("子键不能为Null或空");
        ...
        {
            if (string.IsNullOrEmpty(parentId))
                throw new ArgumentException("父键不能为Null或空");
            if (!_pToC.ContainsKey(parentId))
                throw new Exception("父键不存在");
