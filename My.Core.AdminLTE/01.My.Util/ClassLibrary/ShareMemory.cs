...
namespace My.Util
{
    ...
    public class ShareMemory
    {
        ...
        public ShareMemory(string strName, long lngSize)
        {
    ...
    public class ShareMemory<T>
    {
        ...
        public void Init(string strName, long lngSize)
        {
            ...
            {
                //创建内存共享体(INVALID_HANDLE_VALUE)
                m_hSharedMemoryFile = ShareMemory.CreateFileMapping(INVALID_HANDLE_VALUE, IntPtr.Zero, (uint)PAGE_READWRITE, 0, (uint)lngSize, strName);
                if (m_hSharedMemoryFile == IntPtr.Zero)
                {
                    m_bAlreadyExist = false;
                    m_bInit = false;
                ...
                {
                    if (ShareMemory.GetLastError() == ERROR_ALREADY_EXISTS)  //已经创建
                    {
                        m_bAlreadyExist = true;
                    }
                    else
                ...
                try
                {
                    m_pwData = ShareMemory.MapViewOfFile(m_hSharedMemoryFile, FILE_MAP_WRITE, 0, 0, 0);
                }
                catch (Exception e)
                {
                    Console.WriteLine(e);
                ...
                {
                    m_bInit = false;
                    ShareMemory.CloseHandle(m_hSharedMemoryFile);
                }
                else
                {
                    m_bInit = true;
        ...
        public bool OpenExists(string mapName, long Size)
        {
            try
            {
                m_hSharedMemoryFile = ShareMemory.OpenFileMapping(0x0002, true, mapName);
                m_pwData = ShareMemory.MapViewOfFile(m_hSharedMemoryFile, 0x0002, 0, 0, (uint)Size);
        ...
        public void Close()
        {
            if (m_bInit)
            {
                ShareMemory.UnmapViewOfFile(m_pwData);
                ShareMemory.CloseHandle(m_hSharedMemoryFile);
            }
        }
