...
{
    /// <summary>
    /// 描述：PostgreSql数据库操作帮助类
    /// 作者：wby 2019/9/25 14:49:37
    /// </summary>
    public class PostgreSqlHelper : DbHelper
    {
        ...
        public override List<DbTableInfo> GetDbAllTables(string schemaName = null)
        {
            if (schemaName.IsNullOrEmpty())
                schemaName = "public";
            string sql = @"(select relname as ""TableName"",
                        ...
                        (SELECT viewname as ""TableName"",NULL as ""Description""
                            FROM pg_views WHERE schemaname = @schemaName)";
        ...
        {
            string sql = @"SELECT a.attname as ""Name"", pg_type.typname as ""Type"",
                        (SELECT ""count""(*) from
                        (SELECT ic.column_name as ""ColumnName""
                        FROM
