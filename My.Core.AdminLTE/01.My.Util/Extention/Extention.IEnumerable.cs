...
namespace My.Util
{
    ...
    public static partial class Extention
    {
        ...
        /// <param name="iEnumerable">原数据</param>
        /// <param name="startIndex">原数据开始复制的起始位置</param>
        /// <param name="length">需要复制的数据长度</param>
        /// <returns></returns>
        public static IEnumerable<T> Copy<T>(this IEnumerable<T> iEnumerable, int startIndex, int length)
        {
            var sourceArray = iEnumerable.ToArray();
        ...
        /// <returns></returns>
        public static IEnumerable<T> GetPagination<T>(this IEnumerable<T> iEnumerable, Pagination pagination)
        {
            return iEnumerable.AsQueryable().OrderBy($@"{pagination.SortField} {pagination.SortType}").Skip((pagination.Page - 1) * pagination.Rows).Take(pagination.Rows).ToList();
        }
    }
