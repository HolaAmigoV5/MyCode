...
namespace My.Util
{
    ...
    public static partial class Extention
    {
        ...
        {
            byte[] sh1Bytes = str.ToSHA1Bytes(encoding);
            string resStr = BitConverter.ToString(sh1Bytes);
        ...
        {
            return string.IsNullOrEmpty(jsonStr) ? null : JsonConvert.DeserializeObject<DataTable>(jsonStr);
        }
        ...
        {
            return string.IsNullOrEmpty(jsonStr) ? JObject.Parse("{}") : JObject.Parse(jsonStr.Replace("&nbsp;", ""));
        }
        ...
        /// <returns></returns>
        public static T ToEntity<T>(this string jsonStr)
        {
            ...
            foreach (var item in type.GetProperties())
            {
                ...
                pattern = $"\"{item.Name}\":\"(.*?)\"";
                //pattern = "\"" + item.Name + "\":\"(.*?)\"";
                foreach (Match match in Regex.Matches(jsonStr, pattern))
                {
                    switch (item.PropertyType.ToString())
                    {
                        ...
                        case "System.DateTime":
                            info.SetValue(obj, match.Groups[1].ToString().ToDateTime(), null);
                            break;
                    }
                }
            }
